...
http.createServer(function (request, response) {
	...
	request.on('error', function (err) {
		...
		//	response.end();
		//}
		else if (request.url.includes('loginRecu')) {
			...
			if (body != "") {
				var requestBodyObject = JSON.parse(body);
				if ((requestBodyObject.email != "") && (requestBodyObject.password != "")) {
					var nombre = requestBodyObject.email.split("@");
		...
		else if (request.url.includes('crearNuevoContacto')) {
			...
			if (body != "") {
				var requestBodyObject = JSON.parse(body);
				if (requestBodyObject.nombre != "") {
					requestBodyObject.fechaIngreso = new Date().toLocaleString();
				}
  ...
  //  // response.end(JSON.stringify(responseBody))
		...
		//}
		//else if (request.url.includes('admin.html')) {
